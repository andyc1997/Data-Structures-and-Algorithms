...
def splay(v):
    ...
    while v.parent != None: # If v is not the root
        if v.parent.parent == None: # If v has no grandparent, swap v and v.parent and finish
            smallRotation(v)
            break
        bigRotation(v) # Otherwise, swap v and v.parent and v.parent.parent
    return v

def order_stat(root, key): # Given key, return the order statistic corresponding to the key in a string
    ...
    if v == None: # If v = None, then root = None, nothing we can search for
        return None, root
    while v != None: 
        if v.left != None: # If it has left child, then k-th order statistic is 1 + k-th order statistic of left child
            s = v.left.size + 1
        ...
        if next != None: # Same as v
            if next.left != None:
                ...
                w = 1
        if s >= key and (next == None or s < w): # Initialize next variable for first iteration or the case that key <= s < w
            next = v
        last = v # Update last accessed element in while loop
        if s == key: # The k-th order statistic is found, break the loop and return the found node
            break    
        if s < key: # If s < k, we should search for the right subtree of v and it becomes (k-s)-th order statistic
            key -= s
            v = v.right
        else: # If s > k, we should search for the left subtree of v and no change in order statistic
            v = v.left  
    root = splay(last) # Splay the last accessed node for each loop to maintain splay tree property
    return (next, root)

def split(root, key): # Given root and key, split the splay tree into 2 at the node with given key
    result, root = order_stat(root, key)
    if result == None: # If the given key is greater than all keys in the tree, left tree = current tree, right tree = None
        return (root, None)  
    right = splay(result) # Splay the results up to the root, split the tree at result
    left = right.left # left tree has the root equal right.left, Right tree has the root equal result
    right.left = None
...
def merge(left, right): # Given roots of two trees, merge them into a bigger tree
    ...
    if right == None: # Trivial case
        return left
    while right.left != None: # If the root of right tree has left child, define the root to be its leftmost child
        right = right.left
    right = splay(right) # splay the the root of right tree (in case previous while loop has executed)
    right.left = left # merge two tree (Note that right has no left child), and update sum and children nodes
    update(right)
    return right

def in_order_traversal(root, s): # An iterative algorithm for in-order traversal of BST: https://www.techiedelight.com/inorder-tree-traversal-iterative-recursive/
    v = root
    stack = deque()
    while v != None or len(stack) > 0:
        if v != None:
            ...
            v = stack.pop()
            print(s[v.key - 1], end = '') # Remember to subtract 1 because python string is 0-based but we are using 1-based index
            v = v.right
...
class Rope:
    def __init__(self, s):
        ...
        for ind in range(len(s)):
            insert(ind + 1) # 1-based index. Otherwise, the first character will be put to the end when traversing the tree
        
    def result(self):
        global root
        self.root = root
        in_order_traversal(self.root, self.s) # Print the resulting string during traversing. It keeps O(log(n)) running time
    
    def process(self, i, j, k):
        global root
        middle, right = split(root, j + 1 + 1) # Adjust for 1-based index. As we need to extract S[i...j], the root of right tree is at least j + 1
        left, middle = split(middle, i + 1) # Adjust for 1-based index. As we need to extract S[i...j], the root of middle tree is at least i
        left, right = split(merge(left, right), k + 1) # merge(left, right) gives string with S[i...j] removed. Split it at k so that we can paste before the k-th symbol
        root = merge(merge(left, middle), right) # root of right tree is at least k. So, we are now concatenate S[0...k-1]S[i...j]S[k...len(S) - 1] together.
        return
