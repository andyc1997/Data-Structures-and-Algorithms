...
def tsp_solver(graph):
    ...
    for s in remaining_vertices:
        for S in combinations(remaining_vertices, s):
            ...
            for i, j in product(S, S + (0,)): # Shortest path from vertex 1 to vertex j + 1
                if i != j:
                    current_sol = C[k ^ (1 << i)][j] + graph[j][i] # C(S - {i}, j) + d_{ji}, the path from 1 to i through j
                    if current_sol < C[k][i]: # Check improvement by comparing previous solution C[k][i]
                        C[k][i] = current_sol
