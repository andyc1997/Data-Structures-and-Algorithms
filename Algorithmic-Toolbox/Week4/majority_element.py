...
def get_majority_element(a, left, right):
    ...
    get_maj_list = [element for element in (b, c) if element != -1] # Remove -1 and get_maj_list contains a list of possible major elements

    for maj_element in get_maj_list:
        count = 0
        for i in range(left, right):
            if a[i] == maj_element: # If any element in a[left:right] belongs to major element, count it
                count += 1
        if count > (right - left) / 2: # If the count is already > n/2, return the major element as it's the definition
            return maj_element
