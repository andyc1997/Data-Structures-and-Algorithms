...
def minimum_distance(x, y): # standard implementation of Prim's algorithm
    ...
    for i in range(n): # O(n^2) algorithm to compute distance between points (i.e. edges)
        for j in range(n):
            if i == j:
                adj[i][j] = 0.
            elif i > j:
    ...
    while not H.Empty():
        weight, u = H.Pop() # H pops out the vertex with the smallest cost to add to MST. The first iteration is the first node as cost[0] = 0
        result += weight # add this vertex to MST
        for z in range(len(adj[u])):
            if (z in H.track) and (cost[z] > adj[u][z]): # search its neighborhood and update its cost using information from edges
                cost[z] = adj[u][z]
                H.ChangePriority((cost[z], z)) # update priority and the next iteration will pop out the second lowest cost to add node to vertex u
    return result
