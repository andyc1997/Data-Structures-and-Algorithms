...
def prefix_trie_matching(text, trie):
    ...
    while True:
        if symbol in trie[v]: # If the character is found in the trie, go to the next layer of the trie
            v = trie[v][symbol]
            if len(trie[v]) == 0: # There is a match if it reaches the bottom of the trie, return True
                return True
