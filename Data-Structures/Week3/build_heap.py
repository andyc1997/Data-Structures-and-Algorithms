...
class Heap(): # A binary min-heap
    ...
    def SiftDown(self, i):
        minIndex = i
        l = Heap.LeftChild(i) # Compare if left child has a key less than that of input node.
        if l < len(self.arr):
            if self.arr[l] < self.arr[minIndex]:
                minIndex = l
        r = Heap.RightChild(i)
        if r < len(self.arr):
            if self.arr[r] < self.arr[minIndex]: # Compare if left child has a key less than that of input node or left child.
                minIndex = r
        if i != minIndex:
            self.swap.append((i, minIndex)) # Record swapping operation
            self.arr[i], self.arr[minIndex] = self.arr[minIndex], self.arr[i] # Swap input node with the children with the minimal key
            Heap.SiftDown(self, minIndex) # Do it recursively until no more swapping can be done

def BuildHeap(arr):
    size = len(arr)
    heap = Heap(arr)
    for i in reversed(range(0, size // 2)): # The last n / 2 are leaves. We start at the second last level and push the minimum key up for each subtree
        heap.SiftDown(i)
    return heap.swap
