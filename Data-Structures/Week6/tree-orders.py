...
class TreeOrders:
    def read(self): # Read input and build a binary tree
        ...
        for i in range(self.n):
          [a, b, c] = map(int, sys.stdin.readline().split()) # Update self.key, self.left, self.right from each input line
          self.key[i] = a
          self.left[i] = b
          self.right[i] = c
    ...
    def postOrderTraversal(self, i, result): # Standard implementation of post-order traversal, https://en.wikipedia.org/wiki/Tree_traversal
        if i == -1:
            return None
