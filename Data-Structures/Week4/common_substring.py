...
def compare_length(s, t): # return s and t in which s is always the longer string, an a boolean variable indicates if swapping is applied
...
class hash_table: # A hash table made up of 2 dictionaries (because we're using 2 hash values to reduce collision probability)
    ...
    def clear_cache(self): # In order to control memory, for different lengths of substring in the outer iteration, reset the dictionaries
        self.hash_table_1 = dict()
        self.hash_table_2 = dict()
...
class hashed_string:
    ...
    def hashed_substring(self, start, length): # Return hashed value of substring started at i with length of j, it costs O(1)
...
def match(hashed_s, hashed_t, len_s, len_t, hash_table_t, i, swap):
    for j in range(0, len_t - i + 1): # j is the position at which the substring starts, fill the hash table of shorter string, denoted by t
        h1, h2 = hashed_t.hashed_substring(j, i)
        hash_table_t.update(j, i, h1, h2)
    for j in range(0, len_s - i + 1): # j is the position at which the substring starts, calculate the hash values of longer string, and check if they exist in the hash table of shorter string
        h1, h2 = hashed_s.hashed_substring(j, i) 
        if h1 in hash_table_t.hash_table_1.keys() and h2 in hash_table_t.hash_table_2.keys(): # If so, get the indices and length
            ...
            return answer
    hash_table_t.clear_cache() # Clear hash table to control memory
    return Answer(0, 0, 0) # If hash values do not appear in both dictionaries, return null answer Answer(0, 0, 0)
    
def solve(s, t):
    answer, answer_temp = Answer(0, 0, 0), Answer(0, 0, 0) # Declare two answer variables in this scope
    ...
    hashed_s, hashed_t = hashed_string(s), hashed_string(t) # Do polynomial hashing twice for both strings
    ...
    while right >= left:
        ...
        if answer != Answer(0, 0, 0): # If answer is not null answer, we have found a common substring with length mid. Reduce the search space to [mid + 1, right]
            left = mid + 1
            ...
            right = mid - 1
    if answer_temp != answer and answer == Answer(0, 0, 0): # Return answer_temp if last iteration is indeed null answer
        return answer_temp
