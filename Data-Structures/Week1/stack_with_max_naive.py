...
class StackWithMax():
    ...
    def Push(self, a): # Push: An element is added to the stack
        ...
        if self.__max == []: # If there are no record of maximum value, it is then the maximum
            self.__max.append(a)
        elif a > self.__max[-1]: # If it is greater than the previous maximum value, let it be the maximum
            self.__max.append(a)
    ...
    def Max(self): # It also has O(1) operations
        assert(len(self.__stack))
        return self.__max[-1] # Greedy strategy: Always return the last element. It is a safe move because the last element is always greater than or equal to previous elements
